...
def filter(request):
        ...
        #print results.facets['agent']['terms']
        ...
        for item in results.facets['agent']['terms']:
           ...
           q4 =q4.search()
           q4.facet.facets.append(facets.DateHistogramFacet('date_facet',field='timestamp',interval='hour'))
           results4 = conn.search(query = q4)
           
           finals[count]={u'term' : mystr ,u'entries' : results4.facets.date_facet.entries}
           count = count + 1
        
        finals[0]['entries'].pop(len(finals[0]['entries'])-1)
